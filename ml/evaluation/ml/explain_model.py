...
def main():
    ...
    # This explanation script currently supports the
    # selected Logistic Regression model.
    if not hasattr(model, "coef_"):
        raise ValueError(
            "The selected model does not expose coefficients. "
            "Run train_model.py and confirm Logistic Regression "
    ...
    global_explanation["direction"] = np.where(
        global_explanation["coefficient"] > 0,
        "increases risk",
        "decreases risk",
    )

    global_explanation = global_explanation.sort_values(
        "absolute_importance",
        ascending=False,
    )
    ...
    plot_df = global_explanation.sort_values(
        "absolute_importance",
        ascending=True,
    )
    ...
    ax.set_title(
        "PulseRisk Global Feature Importance"
    )
    ...
    # -----------------------------------------------------
    # Positive / negative coefficient chart
    # -----------------------------------------------------

    direction_df = global_explanation.sort_values(
        "coefficient",
        ascending=True,
    )
    ...
    ax.barh(
        direction_df["feature"],
        direction_df["coefficient"],
    )
    ...
    # Pick examples from low, medium and high predicted risk
    ...
    explanation_json = {
        ...
        "global_feature_effects": [
            ...
            for _, row in global_explanation.iterrows()
        ],
